...
The task is to calculate the sum of f(x, y), g(x, y) and h(x, y) for all integers x and y in the domain D: (1 <= x <= n, 1 <= y <= n).
    ...
    La tarea es calcular la suma de f(x, y), g(x, y) y h(x, y) para todos los enteros x e y en el dominio D: (1 <= x <= n, 1 < = y <= n).

    La función sumin (suma de f) tomará n como parámetro y devolverá la suma de min(x, y) en D.

    La función sumax (suma de g) tomará n como parámetro y devolverá la suma de max(x, y) en D.

    La función sumsum (suma de h) tomará n como parámetro y devolverá la suma de x + y en D.
